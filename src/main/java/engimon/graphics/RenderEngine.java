...
public class RenderEngine {
    ...
    public RenderEngine(Game game) {
        ...
        width = game.getGameVariables().getWindowWidth();
        height = game.getGameVariables().getWindowHeight();
    }

    public synchronized void render() {
        BufferStrategy bufferStrategy = game.getBufferStrategy();
        Graphics graphics = bufferStrategy.getDrawGraphics();
        graphics.setColor(game.getScene().getBackgroundColor());
        graphics.fillRect(0, 0, width, height);
        RelativeGraphics relativeGraphics = new RelativeGraphics(graphics, game.getScene());

        //Render all objects in current scene
        ArrayList<GraphicObject> graphicObjects = game.getScene().getGraphicObjects();
        ...
        String fps = Integer.toString(game.getLastFPS());
